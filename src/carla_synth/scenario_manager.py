...
class NormalDrivingScenarioManager:
    def __init__(
        ...
        avg_vel_threshold=5.0,  # in km/h. This is the threshold for the average velocity during the of the ego vehicle during the recorded sequence to actually save the sample.
        dt=0.01,
        ...
        self.avg_vel_threshold = avg_vel_threshold / 3.6
        ...
        self.traffic_manager.ignore_signs_percentage(self.ego, 100.0)
    ...
    def update(self):
        ...
        if (self.t >= self.sequence_length + self.waittime):
            _save_data = np.mean(self.metadata_recorder.ego_velocities) >= self.avg_vel_threshold
            self.event_recorder.stop(save_data_inst=_save_data)
            self.metadata_recorder.stop(save_data_inst=_save_data)
            if not _save_data:
                print("discarded scenario (not moving enough)")
