...
bitmask_array_current_source = genn_model.create_custom_current_source_class(
    "",
    param_names=["unit_amplitude"],
    var_name_types=[("nt", "int"),
                    ("pop_size", "int")],
    extra_global_params=[("spikeBitmask", "uint32_t*"),
...
threshold_exp_curr = genn_model.create_custom_postsynaptic_class(
    ...
    derived_params=[
        ("expDecay", genn_model.create_dpf_class(
            lambda pars, dt: np.exp(-dt / pars[0]))()),
        ("init", genn_model.create_dpf_class(lambda pars, dt: (
            pars[0] * (1.0 - np.exp(-dt / pars[0]))) * (1.0 / dt))()),
    ],
