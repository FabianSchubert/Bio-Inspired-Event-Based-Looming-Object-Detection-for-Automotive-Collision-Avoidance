...
try:
    ...
    my_font = pygame.font.SysFont('liberationsans', 20)
except ImportError:
...
class World(object):
    ...
    def restart(self):
        ...
        cam_index = self.camera_manager.index if self.camera_manager is not None else 9
        cam_pos_index = (
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    _dt_str = datetime.datetime.now().strftime("%d_%m_%y-%Hh_%Mm_%Ss")
                    _fn_events = os.path.join(BASE_PATH, f"out/events_{_dt_str}.npy")
                    _fn_log = os.path.join(BASE_PATH, f"out/log_{_dt_str}.rec")
                    if world.recording_enabled:
                        client.stop_recorder()
                        world.recording_enabled = False
                    else:
                        client.start_recorder(_fn_log)
                        world.recording_enabled = True
                    print("Hi")
                    world.camera_manager.toggle_recording(_fn_events)
                    ...
                    world.player.set_autopilot(self._autopilot_enabled)
                    world.hud.notification(
                        "Replaying filself.recordinge 'manual_recording.rec'"
                    )
                    # replayer
                    client.replay_file(
...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        if not self._parent.type_id.startswith("walker.pedestrian"):
            self._camera_transforms = [
                ...
                # another ego perspective, closer to the ground
                (
                    carla.Transform(
                        carla.Location(
                            x=+0.05 + bound_x, y=+0.0 * bound_z, z=0.5 * bound_z
                        )
                    ),
                    Attachment.Rigid,
                ),
        ...
        self.append_evt_queue = queue.Queue()
        self.evt_arr_lock = threading.Lock()

        self.append_worker_thread = threading.Thread(target=self.append_worker)
        self.append_worker_thread.daemon = True

        self.append_worker_running = True
        self.append_worker_thread.start()

        self.rec_fn = None

    def append_worker(self):
        while self.append_worker_running:
            while True:
                new_evts = self.append_evt_queue.get()
                if new_evts is None:
                    break
                with self.evt_arr_lock:
                    self.evt_rec_arr = np.append(self.evt_rec_arr, new_evts)
                self.append_evt_queue.task_done()

    def append_evt_array(self, evts):
        self.append_evt_queue.put(evts)
    ...
    def toggle_recording(self, _fn):
        ...
        if self.sensors[self.index][0].startswith("sensor.camera.dvs"):
            if self.recording:
                ...
                self.append_evt_queue.join()
                np.save(self.rec_fn, self.evt_rec_arr)
                del self.evt_rec_arr
    ...
    def _parse_image(weak_self, image):
        ...
        if self.recording:
            if self.sensors[self.index][0].startswith("sensor.camera.dvs"):
                self.append_evt_array(dvs_events)
            else:
...
def game_loop(args):
    ...
    rsn = rsn.to(device)
    rsn.load_state_dict(torch.load("resnet18_atis.pt", map_location=torch.device('cpu')))

    print("connecting to network sim...")
                ...
                for j in range(N_TILES):
                    ...
                    pygame.draw.rect(
                        ...
                        (
                            ...
                            255,
                            255,
                            int(255 * (1.0 - np.exp(-sim_data[i, j] / OUTPUT_SCALE))),
                        ),
                        # (0, 255, 0, 100 * (sim_data[i, j] > THRESHOLD)),
                    ...
                    pygame.draw.rect(
                        draw_surf,
                        # (255, 255, 255, int(255 * (1.-np.exp(-sim_data[i, j]/OUTPUT_SCALE)))),
                        (0, 255, 0, 255 * (sim_data[i, j] > THRESHOLD)),
                        (x0, y0, TILE_WIDTH, TILE_HEIGHT),
                        2,
                    )
                    display.blit(draw_surf, (0, 0))

                    if sim_data[i, j] > THRESHOLD:
                        crop_img = (
                            torch.tensor(
                                world.camera_manager.dvs_img[
...
def main():
    ...
    argparser.add_argument(
        ...
        default="304x240",
        help="window resolution (default: 304x240)",
    )
    argparser.add_argument(
        "--filter",
        metavar="PATTERN",
